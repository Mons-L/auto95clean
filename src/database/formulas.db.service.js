...
const INSERT_FORMULA = 'INSERT INTO formula(label, formulaTypeId) VALUES (?, ?)';
const UPDATE_FORMULA = 'UPDATE formula SET label=?, formulaTypeId=? WHERE id=?';
...
module.exports = {
    ...
    async insertFormula(label, formulaTypeId, transaction){
        let queryMethod = transaction? await db.queryTransaction : db.query
        try{
            let result = await queryMethod(INSERT_FORMULA, [label, formulaTypeId], transaction);
