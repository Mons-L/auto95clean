...
const SELECT_ORDERS_JOIN_ADRESSES_JOIN_CITY_BY_ORDER_ID = 'SELECT a.id, a.fullname, a.number, a.label, a.phone, oa.type addressType, c.label city, c.postalCode FROM orders o INNER JOIN orders_address oa on o.id = oa.orderId INNER JOIN address a on a.id = oa.addressId INNER JOIN city c ON c.id=a.cityId WHERE orderId=?';
